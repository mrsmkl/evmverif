...
Section Basics.
 ...
 Lemma int256_ind_sneakl : forall P : int256->Prop,
  P On ->
 ...
 Lemma int256_ind_twice : forall P : int256->Prop,
  P On ->
  (forall x, P x -> P (twice x)) ->
  (forall x, P x -> P (twice_plus_one x)) ->
  forall x, P x.
 Proof.
 induction x using int256_ind_sneakl; auto.
 destruct d; auto.
 Qed.
 ...
 Variable (A:Type)(case0:A)(caserec:digits->int256->A->A).
 ...
 Fixpoint recrbis_aux (n:nat)(A:Type)(case0:A)(caserec:digits->int256->A->A)
 i : A :=
  match n with
  | O => case0
  | S next =>
 ...
 replace (recrbis_aux n A case0 caserec (I256 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        ...
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0)) with case0; auto.
 clear H IHn; induction n; simpl.
  congruence.
rewrite <- IHn.
auto.
 Qed.
 ...
 Let Incr (b : digits) (si rec : int256) :=
  match b with
   | D0 => sneakl D1 si
   | D1 => sneakl D0 rec
  end.
 ...
 Fixpoint p2ibis n p : (N*int256)%type :=
  match n with
    | O => (Npos p, On)
    | S n => match p with
 ...
 Lemma double_twice_plus_one_firstl : forall x, firstl x = D0 ->
  (Twon*x+In = twice_plus_one x)%int256.
 Proof.
 intros.
 ...
 Lemma positive_to_int256_phi_inv_positive : forall p,
   snd (positive_to_int256 p) = phi_inv_positive p.
 Proof.
 intros; unfold positive_to_int256.
 ...
 Lemma positive_to_int256_spec : forall p,
    Zpos p = (Z.of_N (fst (positive_to_int256 p)))*2^(Z.of_nat size) +
               phi (snd (positive_to_int256 p)).
 Proof.
 unfold positive_to_int256.
...
Definition T2 := add256 In In.

Instance int256_ops : ZnZ.Ops int256 :=
{
 digits      := 256%positive; (* number of digits *)
 zdigits     := T256; (* number of digits *)
 to_Z        := phi; (* conversion to Z *)
 of_pos      := positive_to_int256; (* positive -> N*int31 :  p => N,i
                                      where p = N*2^31+phi i *)
 head0       := head0256;  (* number of head 0 *)
 tail0       := tail0256;  (* number of tail 0 *)
 zero        := On;
 one         := In;
 minus_one   := Tn; (* 2^31 - 1 *)
 compare     := compare256;
 ...
 succ_c      := fun i => i +c In;
 add_c       := add256c;
 add_carry_c := add256carryc;
 succ        := fun i => i + In;
 add         := add256;
 add_carry   := fun i j => i + j + In;
 pred_c      := fun i => i -c In;
 sub_c       := sub256c;
 sub_carry_c := sub256carryc;
 pred        := fun i => i - In;
 sub         := sub256;
 ...
 div21       := div25621;
 div_gt      := div256; (* this is supposed to be the special case of
 ...
 gcd_gt      := gcd256;
 gcd         := gcd256;
 add_mul_div := addmuldiv256;
 pos_mod     := (* modulo 2^p *)
  fun p i =>
  match p ?= T256 with
    | Lt => addmuldiv256 p On (addmuldiv256 (T256-p) i On)
    | _ => i
  end;
 is_even      :=
  fun i => let (_,r) := i/T2 in
...
Section Int256_Specs.
 ...
 Lemma spec_zdigits : [| T256 |] = 256.
 Proof.
  reflexivity.
 Qed.

 Lemma spec_more_than_1_digit: 1 < 256.
 Proof.
  auto with zarith.
 Qed.
 ...
 Lemma spec_add_c  : forall x y, [+|add256c x y|] = [|x|] + [|y|].
 Proof.
 intros; unfold add256c, add256, interp_carry; rewrite phi_phi_inv.
 ...
 Lemma spec_succ_c : forall x, [+|add256c x In|] = [|x|] + 1.
 Proof.
 intros; apply spec_add_c.
 Qed.

 Lemma spec_add_carry_c : forall x y, [+|add256carryc x y|] = [|x|] + [|y|] + 1.
 Proof.
 intros.
 unfold add256carryc, interp_carry; rewrite phi_phi_inv.
 generalize (phi_bounded x)(phi_bounded y); intros.
 set (X:=[|x|]) in *; set (Y:=[|y|]) in *; clearbody X Y.
 ...
 Lemma spec_sub_c : forall x y, [-|sub256c x y|] = [|x|] - [|y|].
 Proof.
 unfold sub256c, sub256, interp_carry; intros.
 rewrite phi_phi_inv.
 generalize (phi_bounded x)(phi_bounded y); intros.
 set (X:=[|x|]) in *; set (Y:=[|y|]) in *; clearbody X Y.
 ...
 Lemma spec_sub_carry_c : forall x y, [-|sub256carryc x y|] = [|x|] - [|y|] - 1.
 Proof.
 unfold sub256carryc, sub256, interp_carry; intros.
 rewrite phi_phi_inv.
 generalize (phi_bounded x)(phi_bounded y); intros.
 set (X:=[|x|]) in *; set (Y:=[|y|]) in *; clearbody X Y.
 ...
 Lemma spec_sub_carry :
   forall x y, [|x-y-In|] = ([|x|] - [|y|] - 1) mod wB.
 Proof.
 unfold sub256; intros.
 repeat rewrite phi_phi_inv.
 apply Zminus_mod_idemp_l.
 Qed.

 Lemma spec_opp_c : forall x, [-|sub256c On x|] = -[|x|].
 Proof.
 ...
 Lemma spec_pred_c : forall x, [-|sub256c x In|] = [|x|] - 1.
 Proof.
 intros; apply spec_sub_c.
 Qed.
 ...
 Lemma spec_mul_c : forall x y, [|| mul256c x y ||] = [|x|] * [|y|].
 Proof.
 unfold mul256c; intros.
 ...
 Lemma spec_square_c : forall x, [|| mul256c x x ||] = [|x|] * [|x|].
 Proof.
 ...
 Lemma spec_div21 : forall a1 a2 b,
      wB/2 <= [|b|] ->
      [|a1|] < [|b|] ->
      let (q,r) := div25621 a1 a2 b in
      [|a1|] *wB+ [|a2|] = [|q|] * [|b|] + [|r|] /\
      0 <= [|r|] < [|b|].
 Proof.
 unfold div25621; intros.
 ...
 Lemma spec_div : forall a b, 0 < [|b|] ->
      let (q,r) := div256 a b in
      [|a|] = [|q|] * [|b|] + [|r|] /\
      0 <= [|r|] < [|b|].
 Proof.
 unfold div256; intros.
 assert ([|b|]>0) by (auto with zarith).
 ...
 Lemma spec_mod :  forall a b, 0 < [|b|] ->
      [|let (_,r) := (a/b)%int256 in r|] = [|a|] mod [|b|].
 Proof.
 unfold div256; intros.
 assert ([|b|]>0) by (auto with zarith).
 unfold Z.modulo.
 ...
 Lemma spec_gcd : forall a b, Zis_gcd [|a|] [|b|] [|gcd256 a b|].
 Proof.
 intros.
 rewrite phi_gcd.
 apply Zis_gcd_sym.
 ...
 Lemma iter_int256_iter_nat : forall A f i a,
  iter_int256 i A f a = iter_nat (Z.abs_nat [|i|]) A f a.
 Proof.
 intros.
 unfold iter_int256.
 ...
 Fixpoint addmuldiv256_alt n i j :=
  match n with
    | O => i
    | S n => addmuldiv256_alt n (sneakl (firstl j) i) (shiftl j)
  end.

 Lemma addmuldiv256_equiv : forall p x y,
  addmuldiv256 p x y = addmuldiv256_alt (Z.abs_nat [|p|]) x y.
 Proof.
 intros.
 unfold addmuldiv256.
 rewrite iter_int256_iter_nat.
 set (n:=Z.abs_nat [|p|]); clearbody n; clear p.
 revert x y; induction n.
 simpl; auto.
 intros.
 simpl addmuldiv256_alt.
 replace (S n) with (n+1)%nat by (rewrite plus_comm; auto).
 rewrite nat_rect_plus; simpl; auto.
 Qed.

 Lemma spec_add_mul_div : forall x y p, [|p|] <= Zpos 256 ->
   [| addmuldiv256 p x y |] =
   ([|x|] * (2 ^ [|p|]) + [|y|] / (2 ^ ((Zpos 256) - [|p|]))) mod wB.
 Proof.
 intros.
 rewrite addmuldiv256_equiv.
 ...
 f_equal.
 ring.
 replace (256-Z.of_nat n) with (Z.succ(256-Z.succ(Z.of_nat n))) by ring.
 ...
 assert ((2*[|y|]) mod wB = 2*[|y|] - wB).
  ...
  f_equal.
 rewrite H1.
 replace wB with (2^(Z.of_nat n)*2^(256-Z.of_nat n)) by
 ...
 Proof.
 unfold int256_ops, ZnZ.pos_mod, compare256.
 change [|T256|] with 256%Z.
 assert (forall w p, 256<=p -> [|w|] = [|w|] mod 2^p).
  intros.
 ...
 intros.
 case_eq ([|p|] ?= 256); intros;
  [ apply H; rewrite (Z.compare_eq _ _ H0); auto with zarith | |
    apply H; change ([|p|]>256)%Z in H0; auto with zarith ].
 change ([|p|]<256) in H0.
 ...
 assert (256-[|p|]<wB).
  apply Z.le_lt_trans with 256%Z; auto with zarith.
  compute; auto.
 assert ([|T256-p|]=256-[|p|]).
  unfold sub256; rewrite phi_phi_inv.
  change [|T256|] with 256%Z.
 ...
 Lemma spec_head00:  forall x, [|x|] = 0 -> [|head0256 x|] = Zpos 256.
 Proof.
  intros.
 ...
 Fixpoint head0256_alt n x :=
  match n with
    | O => 0%nat
    | S n => match firstl x with
               | D0 => S (head0256_alt n (shiftl x))
               | D1 => 0%nat
             end
  end.

 Lemma head0256_equiv :
   forall x, [|head0256 x|] = Z.of_nat (head0256_alt size x).
 Proof.
 intros.
 case_eq (iszero x); intros.
 ...
 unfold head0256, recl.
 change On with (phi_inv (Z.of_nat (256-size))).
 replace (head0256_alt size x) with
   (head0256_alt size x + (256 - size))%nat by auto.
 assert (size <= 256)%nat by auto with arith.

 revert x H; induction size; intros.
 simpl; auto.
 unfold recl_aux; fold recl_aux.
 unfold head0256_alt; fold head0256_alt.
 rewrite H.
 assert ([|phi_inv (Z.of_nat (256-S n))|] = Z.of_nat (256 - S n)).
  rewrite phi_phi_inv.
  apply Zmod_small.
  split.
  change 0 with (Z.of_nat O); apply inj_le; omega.
  apply Z.le_lt_trans with (Z.of_nat 256).
  apply inj_le; omega.
  compute; auto.
 ...
 f_equal.
 change [|In|] with 1.
 replace (256-n)%nat with (S (256 - S n))%nat by omega.
 ...
 Lemma spec_head0  : forall x,  0 < [|x|] ->
	 wB/ 2 <= 2 ^ ([|head0256 x|]) * [|x|] < wB.
 Proof.
 intros.
 rewrite head0256_equiv.
 ...
 rewrite (Nat2Z.inj_succ (head0256_alt n (shiftl x))), Z.pow_succ_r; auto with zarith.
 ...
 change (2^(Z.of_nat 256)/2) with (2^(Z.of_nat (pred size))).
 apply phi_lowerbound; auto.
 Qed.

 Lemma spec_tail00:  forall x, [|x|] = 0 -> [|tail0256 x|] = Zpos 256.
 Proof.
  intros.
  generalize (phi_inv_phi x).
  rewrite H; simpl phi_inv.
 ...
 Fixpoint tail0256_alt n x :=
  match n with
    | O => 0%nat
    | S n => match firstr x with
               | D0 => S (tail0256_alt n (shiftr x))
               | D1 => 0%nat
             end
  end.

 Lemma tail0256_equiv :
   forall x, [|tail0256 x|] = Z.of_nat (tail0256_alt size x).
 Proof.
 intros.
 case_eq (iszero x); intros.
 rewrite (iszero_eq0 _ H).
 ...
 unfold tail0256, recr.
 change On with (phi_inv (Z.of_nat (256-size))).
 replace (tail0256_alt size x) with
   (tail0256_alt size x + (256 - size))%nat by auto.
 assert (size <= 256)%nat by auto with arith.

 revert x H; induction size; intros.
 simpl; auto.
 unfold recr_aux; fold recr_aux.
 unfold tail0256_alt; fold tail0256_alt.
 rewrite H.
 assert ([|phi_inv (Z.of_nat (256-S n))|] = Z.of_nat (256 - S n)).
  rewrite phi_phi_inv.
  apply Zmod_small.
 ...
 Lemma spec_tail0  : forall x, 0 < [|x|] ->
         exists y, 0 <= y /\ [|x|] = (2 * y + 1) * (2 ^ [|tail0256 x|]).
 Proof.
 intros.
 rewrite tail0256_equiv.
 ...
 simpl tail0256_alt.
 case_eq (firstr x); intros.
 rewrite (Nat2Z.inj_succ (tail0256_alt n (shiftr x))), Z.pow_succ_r; auto with zarith.
 ...
 Lemma sqrt256_step_def rec i j:
   sqrt256_step rec i j =
   match (fst (i/j) ?= j)%int256 with
     Lt => rec i (fst ((j + fst(i/j))/T2))%int256
   | _ =>  j
   end.
 Proof.
 unfold sqrt256_step; case div256; intros.
 simpl; case compare256; auto.
 Qed.

 Lemma div256_phi i j: 0 < [|j|] -> [|fst (i/j)%int256|] = [|i|]/[|j|].
 intros Hj; generalize (spec_div i j Hj).
 case div256; intros q r; simpl @fst.
 intros (H1,H2); apply Zdiv_unique with [|r|]; auto with zarith.
 rewrite H1; ring.
 Qed.

 Lemma sqrt256_step_correct rec i j:
  0 < [|i|] -> 0 < [|j|] -> [|i|] < ([|j|] + 1) ^ 2 ->
   2 * [|j|] < wB ->
  (forall j1,
    0 < [|j1|] < [|j|] -> [|i|] < ([|j1|] + 1) ^ 2 ->
    [|rec i j1|] ^ 2 <= [|i|] < ([|rec i j1|] + 1) ^ 2) ->
  [|sqrt256_step rec i j|] ^ 2 <= [|i|] < ([|sqrt256_step rec i j|] + 1) ^ 2.
 Proof.
 assert (Hp2: 0 < [|T2|]) by exact (eq_refl Lt).
 intros Hi Hj Hij H31 Hrec; rewrite sqrt256_step_def.
 rewrite spec_compare, div256_phi; auto.
   case Z.compare_spec; auto; intros Hc;
   try (split; auto; apply sqrt_test_true; auto with zarith; fail).
 apply Hrec; repeat rewrite div256_phi; auto with zarith.
 replace [|(j + fst (i / j)%int256)|] with ([|j|] + [|i|] / [|j|]).
 split.
 ...
 assert (0 <= ([|j|] - 2 + [|i|] / [|j|]) / [|T2|]) ; auto with zarith.
 rewrite <- Hj, Zdiv_1_r.
 replace (1 + [|i|])%Z with (1 * 2 + ([|i|] - 1))%Z; try ring.
 rewrite Z_div_plus_full_l; auto with zarith.
 assert (0 <= ([|i|] - 1) /2)%Z by (apply Z_div_pos; auto with zarith).
 change ([|T2|]) with 2%Z; auto with zarith.
 apply sqrt_test_false; auto with zarith.
 rewrite spec_add, div256_phi; auto.
 symmetry; apply Zmod_small.
 split; auto with zarith.
 replace [|j + fst (i / j)%int256|] with ([|j|] + [|i|] / [|j|]).
 ...
 Lemma iter256_sqrt_correct n rec i j: 0 < [|i|] -> 0 < [|j|] ->
  [|i|] < ([|j|] + 1) ^ 2 -> 2 * [|j|] < 2 ^ (Z.of_nat size) ->
  (forall j1, 0 < [|j1|] -> 2^(Z.of_nat n) + [|j1|] <= [|j|] ->
      [|i|] < ([|j1|] + 1) ^ 2 -> 2 * [|j1|] < 2 ^ (Z.of_nat size) ->
       [|rec i j1|] ^ 2 <= [|i|] < ([|rec i j1|] + 1) ^ 2) ->
  [|iter256_sqrt n rec i j|] ^ 2 <= [|i|] < ([|iter256_sqrt n rec i j|] + 1) ^ 2.
 Proof.
 revert rec i j; elim n; unfold iter256_sqrt; fold iter256_sqrt; clear n.
 intros rec i j Hi Hj Hij H31 Hrec; apply sqrt256_step_correct; auto with zarith.
 intros; apply Hrec; auto with zarith.
 rewrite Z.pow_0_r; auto with zarith.
 intros n Hrec rec i j Hi Hj Hij H31 HHrec.
 apply sqrt256_step_correct; auto.
 ...
 Lemma spec_sqrt : forall x,
       [|sqrt256 x|] ^ 2 <= [|x|] < ([|sqrt256 x|] + 1) ^ 2.
 Proof.
 intros i; unfold sqrt256.
 rewrite spec_compare. case Z.compare_spec; change [|I256 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        ...
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D1|] with 1;intros Hi; auto with zarith.
 repeat rewrite Z.pow_2_r; auto with zarith.
 apply iter256_sqrt_correct; auto with zarith.
  rewrite div256_phi; change ([|I256 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        ...
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D1 D0|]) with 2;  auto with zarith.
 ...
 change ([|I256 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        ...
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0
        D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0 D0|]) with 0; auto with zarith.
 case (phi_bounded i); repeat rewrite Z.pow_2_r; auto with zarith.
 Qed.

 Lemma sqrt2562_step_def rec ih il j:
   sqrt2562_step rec ih il j =
   match (ih ?= j)%int256 with
      Eq => j
    | Gt => j
    | _ =>
       match (fst (div25621 ih il j) ?= j)%int256 with
         Lt => let m := match j +c fst (div25621 ih il j) with
                       C0 m1 => fst (m1/T2)%int256
                     | C1 m1 => (fst (m1/T2) + v255)%int256
                     end in rec ih il m
       | _ =>  j
       end
   end.
 Proof.
 unfold sqrt2562_step; case div25621; intros.
 simpl; case compare256; auto.
 Qed.

 Lemma sqrt2562_lower_bound ih il j:
 ...
 Lemma div2562_phi ih il j: (2^255 <= [|j|] -> [|ih|] < [|j|] ->
  [|fst (div25621 ih il j)|] = phi2 ih il/[|j|])%Z.
 Proof.
 intros Hj Hj1.
 generalize (spec_div21 ih il j Hj Hj1).
 case div25621; intros q r (Hq, Hr).
 apply Zdiv_unique with (phi r); auto with zarith.
 simpl @fst; apply eq_trans with (1 := Hq); ring.
 Qed.

 Lemma sqrt2562_step_correct rec ih il j:
  2 ^ 254 <= [|ih|] -> 0 < [|j|] -> phi2 ih il < ([|j|] + 1) ^ 2 ->
  (forall j1, 0 < [|j1|] < [|j|] ->  phi2 ih il < ([|j1|] + 1) ^ 2 ->
     [|rec ih il j1|] ^ 2 <= phi2 ih il < ([|rec ih il j1|] + 1) ^ 2) ->
  [|sqrt2562_step rec ih il  j|] ^ 2 <= phi2 ih il
      < ([|sqrt2562_step rec ih il j|] + 1) ^  2.
 Proof.
 assert (Hp2: (0 < [|T2|])%Z) by exact (eq_refl Lt).
 intros Hih Hj Hij Hrec; rewrite sqrt2562_step_def.
 assert (H1: ([|ih|] <= [|j|])%Z) by (apply sqrt2562_lower_bound with il; auto).
 ...
 case (Z.le_gt_cases (2 ^ 255) [|j|]); intros Hjj.
 rewrite spec_compare; case Z.compare_spec;
  rewrite div2562_phi; auto; intros Hc;
  try (split; auto; apply sqrt_test_true; auto with zarith; fail).
 apply Hrec.
 ...
 apply sqrt_test_false; auto with zarith.
 generalize (spec_add_c j (fst (div25621 ih il j))).
 unfold interp_carry;  case add256c; intros r;
  rewrite div2562_phi; auto with zarith.
 rewrite div256_phi; change [|T2|] with 2%Z; auto with zarith.
 intros HH; rewrite HH; clear HH; auto with zarith.
 rewrite spec_add, div256_phi; change [|T2|] with 2%Z; auto.
 rewrite Z.mul_1_l; intros HH.
 rewrite Z.add_comm, <- Z_div_plus_full_l; auto with zarith.
 change (phi v255 * 2) with (2 ^ Z.of_nat size).
 rewrite HH, Zmod_small; auto with zarith.
 replace (phi
    match j +c fst (div25621 ih il j) with
    | C0 m1 => fst (m1 / T2)%int256
    | C1 m1 => fst (m1 / T2)%int256 + v255
    end) with ((([|j|] + (phi2 ih il)/([|j|]))/2)).
 apply sqrt_main; auto with zarith.
 generalize (spec_add_c j (fst (div25621 ih il j))).
 unfold interp_carry;  case add256c; intros r;
  rewrite div2562_phi; auto with zarith.
 rewrite div256_phi; auto with zarith.
 intros HH; rewrite HH; auto with zarith.
 intros HH; rewrite <- HH.
 change (1 * 2 ^ Z.of_nat size) with (phi (v255) * 2).
 rewrite Z_div_plus_full_l; auto with zarith.
 rewrite Z.add_comm.
 rewrite spec_add, Zmod_small.
 rewrite div256_phi; auto.
 split; auto with zarith.
 case (phi_bounded (fst (r/T2)%int256));
   case (phi_bounded v255); auto with zarith.
 rewrite div256_phi; change (phi T2) with 2%Z; auto.
 change (2 ^Z.of_nat size) with (base/2 + phi v255).
 ...
 assert ((1 + [|j|]) <= 2 ^ 255); auto with zarith.
 apply Z.le_trans with ((2 ^ 255) * (2 ^ 255)); auto with zarith.
 assert (0 <= 1 + [|j|]); auto with zarith.
 apply Z.mul_le_mono_nonneg; auto with zarith.
 change ((2 ^ 255) * (2 ^ 255)) with ((2 ^ 254) * base).
 ...
 Lemma iter2562_sqrt_correct n rec ih il j:
  2^254 <= [|ih|] ->  0 < [|j|] -> phi2 ih il < ([|j|] + 1) ^ 2 ->
  (forall j1, 0 < [|j1|] -> 2^(Z.of_nat n) + [|j1|] <= [|j|] ->
      phi2 ih il < ([|j1|] + 1) ^ 2 ->
       [|rec ih il j1|] ^ 2 <= phi2 ih il < ([|rec ih il j1|] + 1) ^ 2)  ->
  [|iter2562_sqrt n rec ih il j|] ^ 2 <= phi2 ih il
      < ([|iter2562_sqrt n rec ih il j|] + 1) ^ 2.
 Proof.
 revert rec ih il j; elim n; unfold iter2562_sqrt; fold iter2562_sqrt; clear n.
 intros rec ih il j Hi Hj Hij Hrec; apply sqrt2562_step_correct; auto with zarith.
 intros; apply Hrec; auto with zarith.
 rewrite Z.pow_0_r; auto with zarith.
 intros n Hrec rec ih il j Hi Hj Hij HHrec.
 apply sqrt2562_step_correct; auto.
 intros j1 Hj1  Hjp1; apply Hrec; auto with zarith.
 ...
 Strategy 1 [iter2562_sqrt].

 Lemma spec_sqrt2 : forall x y,
       wB/ 4 <= [|x|] ->
       let (s,r) := sqrt2562 x y in
          [||WW x y||] = [|s|] ^ 2 + [+|r|] /\
          [+|r|] <= 2 * [|s|].
 Proof.
 intros ih il Hih; unfold sqrt2562.
 ...
 { change ((phi Tn + 1) ^ 2) with (2^512).
   apply Z.le_lt_trans with ((2^256 -1) * base + (2^256 - 1)); auto with zarith.
 ...
 case (iter2562_sqrt_correct 256 (fun _ _ j => j) ih il Tn); auto with zarith.
 change [|Tn|] with (2^256 -1).
 assert (1 < 2 ^ 256).
 Search Z.pow.
apply Zpower_gt_1;auto.
omega.
auto.
 intros j1 _ HH; contradict HH.
 apply Z.lt_nge.
 change [|Tn|] with (2^256 -1); auto with zarith.
 change (2 ^ Z.of_nat 256) with (2^256); auto with zarith.
 case (phi_bounded j1); auto with zarith.
 set (s := iter2562_sqrt 256 (fun _ _ j : int256 => j) ih il Tn).
 intros Hs1 Hs2.
 generalize (spec_mul_c s s); case mul256c.
 ...
 { rewrite spec_sub, Zmod_small; auto; change [|In|] with 1.
   ...
   generalize Hih.
 change (2 ^ Z.of_nat size / 4) with 28948022309329048855892746252171976963317496166410141009864396001978282409984.
   split; auto with zarith. }
 rewrite spec_compare; case Z.compare_spec.
 unfold In in Hsih.
 rewrite Hsih.
...
Qed.
 ...
 Proof.
 clear; unfold ZnZ.eq0, int256_ops.
 unfold compare256; intros.
 ...
 Proof.
 unfold ZnZ.is_even, int256_ops; intros.
 generalize (spec_div x T2).
 destruct (x/T2)%int256 as (q,r); intros.
 unfold compare256.
 change [|T2|] with 2 in H.
 change [|On|] with 0.
 ...
 Proof.
 unfold ZnZ.lor,int256_ops. unfold lor256.
 rewrite phi_phi_inv.
 ...
 Proof.
 unfold ZnZ.land, int256_ops. unfold land256.
 rewrite phi_phi_inv.
 apply Z.mod_small; split; trivial.
 ...
 Proof.
 unfold ZnZ.lxor, int256_ops. unfold lxor256.
 rewrite phi_phi_inv.
 apply Z.mod_small; split; trivial.
 ...
 Global Instance int256_specs : ZnZ.Specs int256_ops := {
    spec_to_Z   := phi_bounded;
    spec_of_pos := positive_to_int256_spec;
...
Module Int256Cyclic <: CyclicType.
  Definition t := int256.
  Definition ops := int256_ops.
  Definition specs := int256_specs.
End Int256Cyclic.
