...
Section Basics.
 ...
 Lemma int8_ind_sneakl : forall P : int8->Prop,
  P On ->
 ...
 Lemma int8_ind_twice : forall P : int8->Prop,
  P On ->
  (forall x, P x -> P (twice x)) ->
  (forall x, P x -> P (twice_plus_one x)) ->
  forall x, P x.
 Proof.
 induction x using int8_ind_sneakl; auto.
 destruct d; auto.
 Qed.
 ...
 Variable (A:Type)(case0:A)(caserec:digits->int8->A->A).
 ...
 Fixpoint recrbis_aux (n:nat)(A:Type)(case0:A)(caserec:digits->int8->A->A)
 i : A :=
  match n with
  | O => case0
  | S next =>
 ...
 replace (recrbis_aux n A case0 caserec (I8 D0 D0 D0 D0 D0 D0 D0 D0)) with case0; auto.
...
auto.
 ...
 Fixpoint p2ibis n p : (N*int8)%type :=
  match n with
    | O => (Npos p, On)
    | S n => match p with
 ...
 Lemma double_twice_plus_one_firstl : forall x, firstl x = D0 ->
  (Twon*x+In = twice_plus_one x)%int8.
 Proof.
 intros.
 ...
 Lemma positive_to_int8_phi_inv_positive : forall p,
   snd (positive_to_int8 p) = phi_inv_positive p.
 Proof.
 intros; unfold positive_to_int8.
 ...
 Lemma positive_to_int8_spec : forall p,
    Zpos p = (Z.of_N (fst (positive_to_int8 p)))*2^(Z.of_nat size) +
               phi (snd (positive_to_int8 p)).
 Proof.
 unfold positive_to_int8.
...
Definition T2 := add8 In In.

Instance int8_ops : ZnZ.Ops int8 :=
{
 digits      := 8%positive; (* number of digits *)
 zdigits     := T8; (* number of digits *)
 to_Z        := phi; (* conversion to Z *)
 of_pos      := positive_to_int8; (* positive -> N*int31 :  p => N,i
                                      where p = N*2^31+phi i *)
 head0       := head08;  (* number of head 0 *)
 tail0       := tail08;  (* number of tail 0 *)
 zero        := On;
 one         := In;
 minus_one   := Tn; (* 2^31 - 1 *)
 compare     := compare8;
 ...
 succ_c      := fun i => i +c In;
 add_c       := add8c;
 add_carry_c := add8carryc;
 succ        := fun i => i + In;
 add         := add8;
 add_carry   := fun i j => i + j + In;
 pred_c      := fun i => i -c In;
 sub_c       := sub8c;
 sub_carry_c := sub8carryc;
 pred        := fun i => i - In;
 sub         := sub8;
 ...
 div21       := div821;
 div_gt      := div8; (* this is supposed to be the special case of
 ...
 gcd_gt      := gcd8;
 gcd         := gcd8;
 add_mul_div := addmuldiv8;
 pos_mod     := (* modulo 2^p *)
  fun p i =>
  match p ?= T8 with
    | Lt => addmuldiv8 p On (addmuldiv8 (T8-p) i On)
    | _ => i
  end;
 is_even      :=
...
Section Int8_Specs.
 ...
 Lemma spec_zdigits : [| T8 |] = 8.
 Proof.
  reflexivity.
 Qed.

 Lemma spec_more_than_1_digit: 1 < 8.
 Proof.
  auto with zarith.
 Qed.
 ...
 Lemma spec_add_c  : forall x y, [+|add8c x y|] = [|x|] + [|y|].
 Proof.
 intros; unfold add8c, add8, interp_carry; rewrite phi_phi_inv.
 ...
 Lemma spec_succ_c : forall x, [+|add8c x In|] = [|x|] + 1.
 Proof.
 intros; apply spec_add_c.
 Qed.

 Lemma spec_add_carry_c : forall x y, [+|add8carryc x y|] = [|x|] + [|y|] + 1.
 Proof.
 intros.
 unfold add8carryc, interp_carry; rewrite phi_phi_inv.
 generalize (phi_bounded x)(phi_bounded y); intros.
 set (X:=[|x|]) in *; set (Y:=[|y|]) in *; clearbody X Y.
 ...
 Lemma spec_sub_c : forall x y, [-|sub8c x y|] = [|x|] - [|y|].
 Proof.
 unfold sub8c, sub8, interp_carry; intros.
 rewrite phi_phi_inv.
 generalize (phi_bounded x)(phi_bounded y); intros.
 set (X:=[|x|]) in *; set (Y:=[|y|]) in *; clearbody X Y.
 ...
 Lemma spec_sub_carry_c : forall x y, [-|sub8carryc x y|] = [|x|] - [|y|] - 1.
 Proof.
 unfold sub8carryc, sub8, interp_carry; intros.
 rewrite phi_phi_inv.
 generalize (phi_bounded x)(phi_bounded y); intros.
 set (X:=[|x|]) in *; set (Y:=[|y|]) in *; clearbody X Y.
 ...
 Lemma spec_opp_c : forall x, [-|sub8c On x|] = -[|x|].
 Proof.
 ...
 Lemma spec_pred_c : forall x, [-|sub8c x In|] = [|x|] - 1.
 Proof.
 intros; apply spec_sub_c.
 Qed.
 ...
 Lemma spec_mul_c : forall x y, [|| mul8c x y ||] = [|x|] * [|y|].
 Proof.
 unfold mul8c; intros.
 ...
 Lemma spec_square_c : forall x, [|| mul8c x x ||] = [|x|] * [|x|].
 Proof.
 ...
 Lemma spec_div21 : forall a1 a2 b,
      wB/2 <= [|b|] ->
      [|a1|] < [|b|] ->
      let (q,r) := div821 a1 a2 b in
      [|a1|] *wB+ [|a2|] = [|q|] * [|b|] + [|r|] /\
      0 <= [|r|] < [|b|].
 Proof.
 unfold div821; intros.
 ...
 Lemma spec_gcd : forall a b, Zis_gcd [|a|] [|b|] [|gcd8 a b|].
 Proof.
 intros.
 rewrite phi_gcd.
 apply Zis_gcd_sym.
 ...
 Lemma iter_int8_iter_nat : forall A f i a,
  iter_int8 i A f a = iter_nat (Z.abs_nat [|i|]) A f a.
 Proof.
 intros.
 unfold iter_int8.
 ...
 Fixpoint addmuldiv8_alt n i j :=
  match n with
    | O => i
    | S n => addmuldiv8_alt n (sneakl (firstl j) i) (shiftl j)
  end.

 Lemma addmuldiv8_equiv : forall p x y,
  addmuldiv8 p x y = addmuldiv8_alt (Z.abs_nat [|p|]) x y.
 Proof.
 intros.
 unfold addmuldiv8.
 rewrite iter_int8_iter_nat.
 set (n:=Z.abs_nat [|p|]); clearbody n; clear p.
 revert x y; induction n.
 simpl; auto.
 intros.
 simpl addmuldiv8_alt.
 replace (S n) with (n+1)%nat by (rewrite plus_comm; auto).
 rewrite nat_rect_plus; simpl; auto.
 Qed.

 Lemma spec_add_mul_div : forall x y p, [|p|] <= Zpos 8 ->
   [| addmuldiv8 p x y |] =
   ([|x|] * (2 ^ [|p|]) + [|y|] / (2 ^ ((Zpos 8) - [|p|]))) mod wB.
 Proof.
 intros.
 rewrite addmuldiv8_equiv.
 ...
 f_equal.
 ring.
 replace (8-Z.of_nat n) with (Z.succ(8-Z.succ(Z.of_nat n))) by ring.
 ...
 assert ((2*[|y|]) mod wB = 2*[|y|] - wB).
  ...
  f_equal.
 rewrite H1.
 replace wB with (2^(Z.of_nat n)*2^(8-Z.of_nat n)) by
 ...
 Proof.
 unfold int8_ops, ZnZ.pos_mod, compare8.
 change [|T8|] with 8%Z.
 assert (forall w p, 8<=p -> [|w|] = [|w|] mod 2^p).
  intros.
 ...
 case_eq ([|p|] ?= 8); intros;
  [ apply H; rewrite (Z.compare_eq _ _ H0); auto with zarith | |
    apply H; change ([|p|]>8)%Z in H0; auto with zarith ].
 change ([|p|]<8) in H0.
 ...
 assert (8-[|p|]<wB).
  apply Z.le_lt_trans with 8%Z; auto with zarith.
  compute; auto.
 assert ([|T8-p|]=8-[|p|]).
  unfold sub8; rewrite phi_phi_inv.
  change [|T8|] with 8%Z.
 ...
 Lemma spec_head00:  forall x, [|x|] = 0 -> [|head08 x|] = Zpos 8.
 Proof.
  intros.
 ...
 Fixpoint head08_alt n x :=
  match n with
    | O => 0%nat
    | S n => match firstl x with
               | D0 => S (head08_alt n (shiftl x))
               | D1 => 0%nat
             end
  end.

 Lemma head08_equiv :
   forall x, [|head08 x|] = Z.of_nat (head08_alt size x).
 Proof.
 intros.
 case_eq (iszero x); intros.
 ...
 unfold head08, recl.
 change On with (phi_inv (Z.of_nat (8-size))).
 replace (head08_alt size x) with
   (head08_alt size x + (8 - size))%nat by auto.
 assert (size <= 8)%nat by auto with arith.

 revert x H; induction size; intros.
 simpl; auto.
 unfold recl_aux; fold recl_aux.
 unfold head08_alt; fold head08_alt.
 rewrite H.
 assert ([|phi_inv (Z.of_nat (8-S n))|] = Z.of_nat (8 - S n)).
  rewrite phi_phi_inv.
  apply Zmod_small.
  split.
  change 0 with (Z.of_nat O); apply inj_le; omega.
  apply Z.le_lt_trans with (Z.of_nat 8).
  apply inj_le; omega.
  compute; auto.
 case_eq (firstl x); intros; auto.
 rewrite plus_Sn_m, plus_n_Sm.
 replace (S (8 - S n)) with (8 - n)%nat by omega.
 rewrite <- IHn; [ | omega | ].
 f_equal; f_equal.
 unfold add8.
 rewrite H1.
 f_equal.
 change [|In|] with 1.
 replace (8-n)%nat with (S (8 - S n))%nat by omega.
 ...
 Lemma spec_head0  : forall x,  0 < [|x|] ->
	 wB/ 2 <= 2 ^ ([|head08 x|]) * [|x|] < wB.
 Proof.
 intros.
 rewrite head08_equiv.
 ...
 rewrite (Nat2Z.inj_succ (head08_alt n (shiftl x))), Z.pow_succ_r; auto with zarith.
 ...
 change (2^(Z.of_nat 8)/2) with (2^(Z.of_nat (pred size))).
 apply phi_lowerbound; auto.
 Qed.

 Lemma spec_tail00:  forall x, [|x|] = 0 -> [|tail08 x|] = Zpos 8.
 Proof.
  intros.
  generalize (phi_inv_phi x).
  rewrite H; simpl phi_inv.
 ...
 Fixpoint tail08_alt n x :=
  match n with
    | O => 0%nat
    | S n => match firstr x with
               | D0 => S (tail08_alt n (shiftr x))
               | D1 => 0%nat
             end
  end.

 Lemma tail08_equiv :
   forall x, [|tail08 x|] = Z.of_nat (tail08_alt size x).
 Proof.
 intros.
 case_eq (iszero x); intros.
 rewrite (iszero_eq0 _ H).
 ...
 unfold tail08, recr.
 change On with (phi_inv (Z.of_nat (8-size))).
 replace (tail08_alt size x) with
   (tail08_alt size x + (8 - size))%nat by auto.
 assert (size <= 8)%nat by auto with arith.

 revert x H; induction size; intros.
 simpl; auto.
 unfold recr_aux; fold recr_aux.
 unfold tail08_alt; fold tail08_alt.
 rewrite H.
 assert ([|phi_inv (Z.of_nat (8-S n))|] = Z.of_nat (8 - S n)).
  rewrite phi_phi_inv.
  apply Zmod_small.
 ...
 Lemma spec_tail0  : forall x, 0 < [|x|] ->
         exists y, 0 <= y /\ [|x|] = (2 * y + 1) * (2 ^ [|tail08 x|]).
 Proof.
 intros.
 rewrite tail08_equiv.
 ...
 simpl tail08_alt.
 case_eq (firstr x); intros.
 rewrite (Nat2Z.inj_succ (tail08_alt n (shiftr x))), Z.pow_succ_r; auto with zarith.
 ...
 Lemma sqrt8_step_def rec i j:
   sqrt8_step rec i j =
   match (fst (i/j) ?= j)%int8 with
     Lt => rec i (fst ((j + fst(i/j))/T2))%int8
   | _ =>  j
   end.
 Proof.
 unfold sqrt8_step; case div8; intros.
 simpl; case compare8; auto.
 Qed.

 Lemma div8_phi i j: 0 < [|j|] -> [|fst (i/j)%int8|] = [|i|]/[|j|].
 intros Hj; generalize (spec_div i j Hj).
 case div8; intros q r; simpl @fst.
 intros (H1,H2); apply Zdiv_unique with [|r|]; auto with zarith.
 rewrite H1; ring.
 Qed.

 Lemma sqrt8_step_correct rec i j:
  0 < [|i|] -> 0 < [|j|] -> [|i|] < ([|j|] + 1) ^ 2 ->
   2 * [|j|] < wB ->
  (forall j1,
    0 < [|j1|] < [|j|] -> [|i|] < ([|j1|] + 1) ^ 2 ->
    [|rec i j1|] ^ 2 <= [|i|] < ([|rec i j1|] + 1) ^ 2) ->
  [|sqrt8_step rec i j|] ^ 2 <= [|i|] < ([|sqrt8_step rec i j|] + 1) ^ 2.
 Proof.
 assert (Hp2: 0 < [|T2|]) by exact (eq_refl Lt).
 intros Hi Hj Hij H31 Hrec; rewrite sqrt8_step_def.
 rewrite spec_compare, div8_phi; auto.
   case Z.compare_spec; auto; intros Hc;
   try (split; auto; apply sqrt_test_true; auto with zarith; fail).
 apply Hrec; repeat rewrite div8_phi; auto with zarith.
 replace [|(j + fst (i / j)%int8)|] with ([|j|] + [|i|] / [|j|]).
 split.
 ...
 rewrite spec_add, div8_phi; auto.
 symmetry; apply Zmod_small.
 split; auto with zarith.
 replace [|j + fst (i / j)%int8|] with ([|j|] + [|i|] / [|j|]).
 ...
 Lemma iter8_sqrt_correct n rec i j: 0 < [|i|] -> 0 < [|j|] ->
  [|i|] < ([|j|] + 1) ^ 2 -> 2 * [|j|] < 2 ^ (Z.of_nat size) ->
  (forall j1, 0 < [|j1|] -> 2^(Z.of_nat n) + [|j1|] <= [|j|] ->
      [|i|] < ([|j1|] + 1) ^ 2 -> 2 * [|j1|] < 2 ^ (Z.of_nat size) ->
       [|rec i j1|] ^ 2 <= [|i|] < ([|rec i j1|] + 1) ^ 2) ->
  [|iter8_sqrt n rec i j|] ^ 2 <= [|i|] < ([|iter8_sqrt n rec i j|] + 1) ^ 2.
 Proof.
 revert rec i j; elim n; unfold iter8_sqrt; fold iter8_sqrt; clear n.
 intros rec i j Hi Hj Hij H31 Hrec; apply sqrt8_step_correct; auto with zarith.
 intros; apply Hrec; auto with zarith.
 rewrite Z.pow_0_r; auto with zarith.
 intros n Hrec rec i j Hi Hj Hij H31 HHrec.
 apply sqrt8_step_correct; auto.
 ...
 Lemma spec_sqrt : forall x,
       [|sqrt8 x|] ^ 2 <= [|x|] < ([|sqrt8 x|] + 1) ^ 2.
 Proof.
 intros i; unfold sqrt8.
 rewrite spec_compare. case Z.compare_spec; change [|I8 D0 D0 D0 D0 D0 D0 D0 D1|] with 1;intros Hi; auto with zarith.
 repeat rewrite Z.pow_2_r; auto with zarith.
 apply iter8_sqrt_correct; auto with zarith.
  rewrite div8_phi; change ([|I8 D0 D0 D0 D0 D0 D0 D1 D0|]) with 2;  auto with zarith.
 ...
 change [|I8 D0 D0 D0 D0 D0 D0 D0 D0|] with 0; auto with zarith.
 case (phi_bounded i); repeat rewrite Z.pow_2_r; auto with zarith.
 Qed.

 Lemma sqrt82_step_def rec ih il j:
   sqrt82_step rec ih il j =
   match (ih ?= j)%int8 with
      Eq => j
    | Gt => j
    | _ =>
       match (fst (div821 ih il j) ?= j)%int8 with
         Lt => let m := match j +c fst (div821 ih il j) with
                       C0 m1 => fst (m1/T2)%int8
                     | C1 m1 => (fst (m1/T2) + v7)%int8
                     end in rec ih il m
       | _ =>  j
       end
   end.
 Proof.
 unfold sqrt82_step; case div821; intros.
 simpl; case compare8; auto.
 Qed.

 Lemma sqrt82_lower_bound ih il j:
 ...
 Lemma div82_phi ih il j: (2^7 <= [|j|] -> [|ih|] < [|j|] ->
  [|fst (div821 ih il j)|] = phi2 ih il/[|j|])%Z.
 Proof.
 intros Hj Hj1.
 generalize (spec_div21 ih il j Hj Hj1).
 case div821; intros q r (Hq, Hr).
 apply Zdiv_unique with (phi r); auto with zarith.
 simpl @fst; apply eq_trans with (1 := Hq); ring.
 Qed.

 Lemma sqrt82_step_correct rec ih il j:
  2 ^ 6 <= [|ih|] -> 0 < [|j|] -> phi2 ih il < ([|j|] + 1) ^ 2 ->
  (forall j1, 0 < [|j1|] < [|j|] ->  phi2 ih il < ([|j1|] + 1) ^ 2 ->
     [|rec ih il j1|] ^ 2 <= phi2 ih il < ([|rec ih il j1|] + 1) ^ 2) ->
  [|sqrt82_step rec ih il  j|] ^ 2 <= phi2 ih il
      < ([|sqrt82_step rec ih il j|] + 1) ^  2.
 Proof.
 assert (Hp2: (0 < [|T2|])%Z) by exact (eq_refl Lt).
 intros Hih Hj Hij Hrec; rewrite sqrt82_step_def.
 assert (H1: ([|ih|] <= [|j|])%Z) by (apply sqrt82_lower_bound with il; auto).
 ...
 case (Z.le_gt_cases (2 ^ 7) [|j|]); intros Hjj.
 rewrite spec_compare; case Z.compare_spec;
  rewrite div82_phi; auto; intros Hc;
  try (split; auto; apply sqrt_test_true; auto with zarith; fail).
 apply Hrec.
 ...
 apply sqrt_test_false; auto with zarith.
 generalize (spec_add_c j (fst (div821 ih il j))).
 unfold interp_carry;  case add8c; intros r;
  rewrite div82_phi; auto with zarith.
 rewrite div8_phi; change [|T2|] with 2%Z; auto with zarith.
 intros HH; rewrite HH; clear HH; auto with zarith.
 rewrite spec_add, div8_phi; change [|T2|] with 2%Z; auto.
 rewrite Z.mul_1_l; intros HH.
 rewrite Z.add_comm, <- Z_div_plus_full_l; auto with zarith.
 change (phi v7 * 2) with (2 ^ Z.of_nat size).
 rewrite HH, Zmod_small; auto with zarith.
 replace (phi
    match j +c fst (div821 ih il j) with
    | C0 m1 => fst (m1 / T2)%int8
    | C1 m1 => fst (m1 / T2)%int8 + v7
    end) with ((([|j|] + (phi2 ih il)/([|j|]))/2)).
 apply sqrt_main; auto with zarith.
 generalize (spec_add_c j (fst (div821 ih il j))).
 unfold interp_carry;  case add8c; intros r;
  rewrite div82_phi; auto with zarith.
 rewrite div8_phi; auto with zarith.
 intros HH; rewrite HH; auto with zarith.
 intros HH; rewrite <- HH.
 change (1 * 2 ^ Z.of_nat size) with (phi (v7) * 2).
 rewrite Z_div_plus_full_l; auto with zarith.
 rewrite Z.add_comm.
 rewrite spec_add, Zmod_small.
 rewrite div8_phi; auto.
 split; auto with zarith.
 case (phi_bounded (fst (r/T2)%int8));
   case (phi_bounded v7); auto with zarith.
 rewrite div8_phi; change (phi T2) with 2%Z; auto.
 change (2 ^Z.of_nat size) with (base/2 + phi v7).
 ...
 assert ((1 + [|j|]) <= 2 ^ 7); auto with zarith.
 apply Z.le_trans with ((2 ^ 7) * (2 ^ 7)); auto with zarith.
 assert (0 <= 1 + [|j|]); auto with zarith.
 apply Z.mul_le_mono_nonneg; auto with zarith.
 change ((2 ^ 7) * (2 ^ 7)) with ((2 ^ 6) * base).
 ...
 Lemma iter82_sqrt_correct n rec ih il j:
  2^6 <= [|ih|] ->  0 < [|j|] -> phi2 ih il < ([|j|] + 1) ^ 2 ->
  (forall j1, 0 < [|j1|] -> 2^(Z.of_nat n) + [|j1|] <= [|j|] ->
      phi2 ih il < ([|j1|] + 1) ^ 2 ->
       [|rec ih il j1|] ^ 2 <= phi2 ih il < ([|rec ih il j1|] + 1) ^ 2)  ->
  [|iter82_sqrt n rec ih il j|] ^ 2 <= phi2 ih il
      < ([|iter82_sqrt n rec ih il j|] + 1) ^ 2.
 Proof.
 revert rec ih il j; elim n; unfold iter82_sqrt; fold iter82_sqrt; clear n.
 intros rec ih il j Hi Hj Hij Hrec; apply sqrt82_step_correct; auto with zarith.
 intros; apply Hrec; auto with zarith.
 rewrite Z.pow_0_r; auto with zarith.
 intros n Hrec rec ih il j Hi Hj Hij HHrec.
 apply sqrt82_step_correct; auto.
 intros j1 Hj1  Hjp1; apply Hrec; auto with zarith.
 ...
 Strategy 1 [iter82_sqrt].

 Lemma spec_sqrt2 : forall x y,
       wB/ 4 <= [|x|] ->
       let (s,r) := sqrt82 x y in
          [||WW x y||] = [|s|] ^ 2 + [+|r|] /\
          [+|r|] <= 2 * [|s|].
 Proof.
 intros ih il Hih; unfold sqrt82.
 ...
 { change ((phi Tn + 1) ^ 2) with (2^16).
   apply Z.le_lt_trans with ((2^8 -1) * base + (2^8 - 1)); auto with zarith.
 ...
 case (iter82_sqrt_correct 8 (fun _ _ j => j) ih il Tn); auto with zarith.
 change [|Tn|] with 255; auto with zarith.
 intros j1 _ HH; contradict HH.
 apply Z.lt_nge.
 change [|Tn|] with 255; auto with zarith.
 change (2 ^ Z.of_nat 8) with 256; auto with zarith.
 case (phi_bounded j1); auto with zarith.
 set (s := iter82_sqrt 8 (fun _ _ j : int8 => j) ih il Tn).
 intros Hs1 Hs2.
 generalize (spec_mul_c s s); case mul8c.
 ...
 { rewrite spec_sub, Zmod_small; auto; change [|In|] with 1.
   ...
   generalize Hih.
 change (2 ^ Z.of_nat size / 4) with 64.
   split; auto with zarith. }
 rewrite spec_compare; case Z.compare_spec.
 unfold In in Hsih.
 rewrite Hsih.
...
Qed.
 ...
 Proof.
 clear; unfold ZnZ.eq0, int8_ops.
 unfold compare8; intros.
 ...
 Proof.
 unfold ZnZ.is_even, int8_ops; intros.
 generalize (spec_div x T2).
 destruct (x/T2)%int8 as (q,r); intros.
 unfold compare8.
 ...
 Proof.
 unfold ZnZ.lor,int8_ops. unfold lor8.
 rewrite phi_phi_inv.
 ...
 Proof.
 unfold ZnZ.land, int8_ops. unfold land8.
 rewrite phi_phi_inv.
 apply Z.mod_small; split; trivial.
 ...
 Proof.
 unfold ZnZ.lxor, int8_ops. unfold lxor8.
 rewrite phi_phi_inv.
 apply Z.mod_small; split; trivial.
 ...
 Global Instance int8_specs : ZnZ.Specs int8_ops := {
    spec_to_Z   := phi_bounded;
    spec_of_pos := positive_to_int8_spec;
...
Module Int8Cyclic <: CyclicType.
  Definition t := int8.
  Definition ops := int8_ops.
  Definition specs := int8_specs.
End Int8Cyclic.
